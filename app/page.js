...
function GameCardIcon({ game, size = 28 }) {
  ...
  if (!iconName) {
    if (slug.includes('kuis') || slug.includes('quiz')) iconName = 'quiz';
...
export default async function HomePage() {
  ...
  // Filter out 'games' card from tools list, take top 10
  const essentialTools = safeTools.filter((t) => t.slug !== 'games').slice(0, 10);
  ...
  return (
    <div className="shell">
      <section className="hero">
        <span className="hero-badge simple-only">{essentialTools.length} tools · {safeGames.length} game</span>
        <span className="hero-badge pro-only">Neuro Core Digital Arsenal</span>
        <h1><span className="grad">Tools yang beneran gampang dipakai</span></h1>
      ...
      <section>
        ...
        <div className="grid-cards">
          {gamePreview.map((game, i) => (
            <Link key={game.slug || `${game.name}-${i}`} href={`/games/${game.slug}`} className="card">
        ...
        <div className="panel">
          {top3.length > 0 ? (
            <div style={{ display: 'grid', gap: 8 }}>
              {top3.map((row, i) => (
                <div key={`${row.name}-${i}`} className={`rank-row ${i === 0 ? 'gold' : i === 1 ? 'silver' : 'bronze'}`}>
                  <span style={{ minWidth: 36, fontWeight: 800, color: 'var(--text-faint)' }}>#{i + 1}</span>
                  <span style={{ flex: 1, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.name}</span>
            ...
            <p style={{ margin: 0, color: 'var(--text-dim)', fontSize: 14 }}>
              Belum ada skor. <Link href="/games" style={{ color: 'var(--accent-soft)', fontWeight: 600 }}>Main game</Link> dan jadi yang pertama.
            </p>
          )}
        </div>
      </section>
