...
export default function StickerMakerPage() {
  ...
  const [sticker, setSticker] = useState(null); // { url, blob, size }
  ...
  const build = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          // Safari lama tidak dukung webp -> fallback PNG
      ...
      <div className="panel">
        ...
        {sticker ? (
          <div className="result" style={{ textAlign: 'center' }}>
            ...
            <p className="hint">
              Di WhatsApp: buka chat → 😊 → Stiker → ➕ → pilih file ini.
            </p>
          </div>
        ) : null}
      </div>
